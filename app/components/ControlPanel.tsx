...
export function ControlPanel({onChange,currentSub}:{onChange:()=>void;currentSub:string}) {
  ...
  return <section className="panel control-panel"><div className="control-heading"><h2>{c.title}</h2><button className="button" disabled={busy} onClick={()=>void load()}>{c.reload}</button></div>
    ...
    {config ? <form hidden={section!=='config'} onInvalidCapture={e=>{const details=(e.target as HTMLElement).closest('details');if(details)details.open=true;}} onSubmit={e=>{e.preventDefault();if(config.targets.some(t=>!config.allowedApiIds.includes(t.apiId))){setMessage(c.selectApi);return;}void save('config',config);}}>
